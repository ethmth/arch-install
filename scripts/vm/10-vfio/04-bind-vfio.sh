...
sudo sh -c "echo \"#!/bin/bash\" > /usr/bin/gpustart"
sudo sh -c "echo \"echo \"Edit this script (/usr/bin/gpustart) and keep whichever method works\"\" > /usr/bin/gpustart"
...
sudo sh -c "echo \"lspci -nnk -d $gpu_pci_id | grep Kernel\" >> /usr/bin/gpucheck"
