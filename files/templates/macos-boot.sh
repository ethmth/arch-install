...
VM_DIR="/home/e/vm"
MY_PATH="$VM_DIR/osx/MacVentura"
REPO_PATH="$MY_PATH/osx-serial-generator/OSX-KVM"
OVMF_DIR="."
...
args=(
  ...
  -device vfio-pci,host=$PCI_VIDEO,multifunction=on,x-no-kvm-intx=on
  # -device vfio-pci,host=01:00.0,multifunction=on,romfile=gpu_original_bios.bin
  -device vfio-pci,host=$PCI_AUDIO
  ...
  -drive id=OpenCoreBoot,if=none,snapshot=on,format=qcow2,file="$MY_PATH/OpenCore.qcow2"
  -device ide-hd,bus=sata.2,drive=OpenCoreBoot
  -device ide-hd,bus=sata.3,drive=InstallMedia
  -drive id=InstallMedia,if=none,file="$VM_DIR/os/macos-Ventura.img",format=raw
  -drive id=MacHDD,if=none,file="$VM_DIR/disk/MacVentura.qcow2",format=qcow2
